...
public abstract class Surf extends Draw {
	...
	static class gllightmapstate_t
	{
		...
		ByteBuffer/*IntBuffer*/ lightmap_buffer = Lib.newIntBuffer(BLOCK_WIDTH * BLOCK_HEIGHT, ByteOrder.LITTLE_ENDIAN);
	...
	abstract void R_BuildLightMap(msurface_t surf, ByteBuffer/*IntBuffer*/ dest, int stride);
	...
	*/
	...
	private final ByteBuffer/*IntBuffer*/ temp2 = Lib.newIntBuffer(34 * 34, ByteOrder.LITTLE_ENDIAN);
		...
		*/
		...
		if ( is_dynamic )
		{
			if ( ( (fa.styles[maps] & 0xFF) >= 32 || fa.styles[maps] == 0 ) && ( fa.dlightframe != r_framecount ) )
			{
				...
				glTexSubImage2D( GL11.GL_TEXTURE_2D, 0,
	...
	void R_DrawAlphaSurfaces()
	{
		...
		glInterleavedArrays(GL11.GL_T2F_V3F, Polygon.BYTE_STRIDE, globalPolygonInterleavedBuf);
	...
	// direct buffer
	private final ByteBuffer/*IntBuffer*/ temp = Lib.newIntBuffer(128 * 128, ByteOrder.LITTLE_ENDIAN);
	...
	void GL_RenderLightmappedPoly( msurface_t surf )
	{
		...
		glpoly_t p;
		ByteBuffer/*FloatBuffer*/ texCoord = globalPolygonInterleavedBuf;
	...
	void R_DrawBrushModel(entity_t e)
	{
		...
		GL_TexEnv( GL11.GL_MODULATE );
		glTexCoordPointerF(2, Polygon.BYTE_STRIDE, globalPolygonTexCoord1Buf);
	...
	*/
	...
	void LM_UploadBlock( boolean dynamic )
	{
		...
		glTexParameterf(GL11.GL_TEXTURE_2D, GL11.GL_TEXTURE_MIN_FILTER, GL11.GL_LINEAR);
		glTexParameterf(GL11.GL_TEXTURE_2D, GL11.GL_TEXTURE_MAG_FILTER, GL11.GL_LINEAR);
		...
		if ( dynamic )
		{
			...
			glTexSubImage2D( GL11.GL_TEXTURE_2D,
							  0,
							  0, 0,
							  BLOCK_WIDTH, height,
							  GL_LIGHTMAP_FORMAT,
	...
	void GL_CreateSurfaceLightmap(msurface_t surf)
	{
		...
		ByteBuffer/*IntBuffer*/ base = gl_lms.lightmap_buffer;
	 ...
	 */
	static ByteBuffer/*FloatBuffer*/ globalPolygonInterleavedBuf = Polygon.getInterleavedBuffer();
	static ByteBuffer/*FloatBuffer*/ globalPolygonTexCoord1Buf = null;
