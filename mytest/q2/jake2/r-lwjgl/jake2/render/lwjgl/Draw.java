...
// ====================================================================
    ...
    // share the buffer
    private ByteBuffer/*IntBuffer*/ image32 = image8;//.asIntBuffer();
	...
	protected void Draw_StretchRaw (int x, int y, int w, int h, int cols, int rows, byte[] data)
	{
		...
		if ( !qglColorTableEXT )
		{
			...
			for (i=0 ; i<trows ; i++)
			{
				...
				for (j=0 ; j<256 ; j++)
				{
					image32.putInt(destIndex + j, r_rawpalette[data[sourceIndex + (frac>>16)] & 0xff]);
					frac += fracstep;
				}
			}
			glTexImage2D (GL11.GL_TEXTURE_2D, 0, gl_tex_solid_format, 256, 256, 0, GL11.GL_RGBA, GL11.GL_UNSIGNED_BYTE, image32);
		}
		else
		{
