 ...
 * Copyright (C) 2003
 *
 * $Id: Polygon.java,v 1.2 2005/01/10 00:12:00 cawe Exp $
 */
/*
...
public final class Polygon extends glpoly_t {
	...
	private static ByteBuffer/*FloatBuffer*/ buffer = Lib.newFloatBuffer(MAX_BUFFER_VERTICES * STRIDE);
	...
	static ByteBuffer/*FloatBuffer*/ getInterleavedBuffer() {
	    return (ByteBuffer)/*(FloatBuffer)*/buffer.rewind();
	}
	...
	public final float x(int index) {
	    return buffer.getFloat((index + pos) * 7 + 2);
	}

	public final void x(int index, float value) {
	    buffer.putFloat((index + pos) * 7 + 2, value);
	}

	public final float y(int index) {
	    return buffer.getFloat((index + pos) * 7 + 3);
	}

	public final void y(int index, float value) {
	    buffer.putFloat((index + pos) * 7 + 3, value);
	}

	public final float z(int index) {
	    return buffer.getFloat((index + pos) * 7 + 4);
	}

	public final void z(int index, float value) {
	    buffer.putFloat((index + pos) * 7 + 4, value);
	}

	public final float s1(int index) {
	    return buffer.getFloat((index + pos) * 7 + 0);
	}

	public final void s1(int index, float value) {
	    buffer.putFloat((index + pos) * 7 + 0, value);
	}

	public final float t1(int index) {
	    return buffer.getFloat((index + pos) * 7 + 1);
	}

	public final void t1(int index, float value) {
	    buffer.putFloat((index + pos) * 7 + 1, value);
	}

	public final float s2(int index) {
	    return buffer.getFloat((index + pos) * 7 + 5);
	}

	public final void s2(int index, float value) {
	    buffer.putFloat((index + pos) * 7 + 5, value);
	}

	public final float t2(int index) {
	    return buffer.getFloat((index + pos) * 7 + 6);
	}

	public final void t2(int index, float value) {
	    buffer.putFloat((index + pos) * 7 + 6, value);
	}

	public final void beginScrolling(float scroll) {
	    int index = pos * 7;
	    for (int i = 0; i < numverts; i++, index+=7) {
	        scroll += s1_old[i] = buffer.getFloat(index);
            buffer.putFloat(index, scroll);
        }
	}
	...
	public final void endScrolling() {
	    int index = pos * 7;
	    for (int i = 0; i < numverts; i++, index+=7) {
            buffer.putFloat(index, s1_old[i]);
        }
	}
}
