...
public abstract class Image extends Main {
	...
	void GL_SetTexturePalette(int[] palette) {
		...
		if (qglColorTableEXT && gl_ext_palettedtexture.value != 0.0f)
		{
			...
			//TODO: FIXME:
//			glColorTable(EXTSharedTexturePalette.GL_SHARED_TEXTURE_PALETTE_EXT, GL11.GL_RGB, 256, GL11.GL_RGB, GL11.GL_UNSIGNED_BYTE, temptable);
		}
	}
	...
	void GL_TexEnv(int mode /* GLenum */
		...
		if (mode != lastmodes[gl_state.currenttmu]) {
			glTexEnvi(GL11.GL_TEXTURE_ENV, GL11.GL_TEXTURE_ENV_MODE, mode);
	...
	void GL_Bind(int texnum) {
		...
		glBindTexture(GL11.GL_TEXTURE_2D, texnum);
	}
	...
	void GL_TextureMode(String string) {
		...
		for (i = 0; i < numgltextures; i++) {
			...
			if (glt.type != it_pic && glt.type != it_sky) {
				GL_Bind(glt.texnum);
				glTexParameteri(GL11.GL_TEXTURE_2D, GL11.GL_TEXTURE_MIN_FILTER, gl_filter_min);
				glTexParameteri(GL11.GL_TEXTURE_2D, GL11.GL_TEXTURE_MAG_FILTER, gl_filter_max);
			}
		}
	}
...
//		}
	...
	*/
	...
	ByteBuffer/*IntBuffer*/ tex = Lib.newIntBuffer(512 * 256, ByteOrder.LITTLE_ENDIAN);
	...
	boolean GL_Upload32(int[] data, int width, int height, boolean mipmap) {
		...
		try {
			if (scaled_width == width && scaled_height == height) {
				if (!mipmap) {
					if (qglColorTableEXT && gl_ext_palettedtexture.value != 0.0f && samples == gl_solid_format) {
						...
						tex.rewind(); for(int j : data) tex.putInt(j)/*tex.put(data)*/; tex.rewind();
						glTexImage2D(
							GL11.GL_TEXTURE_2D,
							0,
							comp,
			...
			if (qglColorTableEXT && gl_ext_palettedtexture.value != 0.0f && (samples == gl_solid_format)) {
				...
				tex.rewind(); for(int j : scaled) tex.putInt(j)/*tex.put(scaled)*/; tex.rewind();
				glTexImage2D(GL11.GL_TEXTURE_2D, 0, comp, scaled_width, scaled_height, 0, GL11.GL_RGBA, GL11.GL_UNSIGNED_BYTE, tex);
			}
		...
		if (mipmap) {
			glTexParameterf(GL11.GL_TEXTURE_2D, GL11.GL_TEXTURE_MIN_FILTER, gl_filter_min);
			glTexParameterf(GL11.GL_TEXTURE_2D, GL11.GL_TEXTURE_MAG_FILTER, gl_filter_max);
		}
		else {
			glTexParameterf(GL11.GL_TEXTURE_2D, GL11.GL_TEXTURE_MIN_FILTER, gl_filter_max);
			glTexParameterf(GL11.GL_TEXTURE_2D, GL11.GL_TEXTURE_MAG_FILTER, gl_filter_max);
		}
	...
	*/
	...
	boolean GL_Upload8(byte[] data, int width, int height, boolean mipmap, boolean is_sky) {
		...
		if (qglColorTableEXT && gl_ext_palettedtexture.value != 0.0f && is_sky) {
			glTexImage2D(GL11.GL_TEXTURE_2D, 0, GL_COLOR_INDEX8_EXT, width, height, 0, GL11.GL_COLOR_INDEX, GL11.GL_UNSIGNED_BYTE, ByteBuffer.wrap(data));
